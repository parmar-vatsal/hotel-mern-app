...
const RoomDetailsScreen = () => {
    ...
    useEffect(() => {
        if (successBookingCreate) {
            dispatch(getBookedDates(id as string)); // Refresh booked dates to update calendar
            setShowSuccessModal(true);
        }
    }, [successBookingCreate, dispatch, id]);
                ...
                <Col>
                                ...
                                <Card.Body>
                                    ...
                                    {errorRoomIsAvailable && <Message variant="danger">{errorRoomIsAvailable}</Message>}
                                     ...
                                     {loggedIn && successRoomIsAvailable && ( 
                                        <Button size="lg" variant="primary" className="mb-3" onClick={() => setShowBookingModal(true)}>
                                            Book Now ${Number(room.pricePerNight) * Number(daysOfStay)}
                                        </Button>
                                     )}

                                     {!loggedIn && successRoomIsAvailable && (
                                        <Button size="lg" variant="primary" className="mb-3" onClick={() => setShowBookingModal(true)}>
                                            Book as Guest ${Number(room.pricePerNight) * Number(daysOfStay)}
                                        </Button>
                                     )}
                    ...
                    <Modal show={showBookingModal} onHide={() => setShowBookingModal(false)} centered>
                        <Modal.Header closeButton>
                            <Modal.Title>Guest Checkout</Modal.Title>
                        </Modal.Header>
                        <Modal.Body>
                            <BookingForm
                                room={room}
