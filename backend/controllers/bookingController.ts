...
export const newBooking = asyncHandler(async (req: IUserRequest, res: Response) => {
    ...
    const { room, checkInDate, checkOutDate, amountPaid, daysOfStay, paymentInfo, guestName, guestEmail, guestPhone, specialRequests } = req.body;

    // Validate required fields
    if (!room || !checkInDate || !checkOutDate || !amountPaid || !daysOfStay) {
        res.status(400);
...
export const myBookings = asyncHandler(async (req: IUserRequest, res: Response) => {
    ...
    const bookings = await Booking.find({ $or: [{ user: req.user._id }, { guestEmail: req.user.email }] }).populate("user", "name email").populate("room", "name images");
...
// @Method DELETE
export const deleteBooking = asyncHandler(async (req: IUserRequest, res: Response) => {
